...
public:
    ...
    static bool verify_tag_constant_time(const array<uint8_t, 16>& tag1,
                                         const array<uint8_t, 16>& tag2)
    {
        uint8_t diff = 0;
...
static void print_hex(const string& label, const uint8_t* data, size_t len)
{
...
int main()
{
    ...
    array<uint8_t, 12> iv = {
        0xfe, 0xed, 0xfa, 0xce, 0xde, 0xad,
        0xbe, 0xef, 0x10, 0x20, 0x30, 0x40
    };

    
    string plain_str = u8"Этот текст надо зашифровать по-хорошему";
    string aad_str   = u8"Заголовки";

    vector<uint8_t> plaintext(plain_str.begin(), plain_str.end());
    vector<uint8_t> aad(aad_str.begin(), aad_str.end());

    cout << "=== ИСХОДНЫЕ ДАННЫЕ ===" << endl;
    cout << "Открытый текст: " << plain_str << endl;
    cout << "Длина в байтах: " << plaintext.size() << endl;
    ...
    cout << "=== ПРОВЕРКА GF(2^8), p(x)=0x11D ===" << endl;
    cout << "S(0x00) = 0x"
         << hex << setw(2) << setfill('0')
         << static_cast<int>(gf.inverse(0x00) == 0 ? 0x63 : 0x00)
         << " (должно быть 0x63 через аффинное)" << endl;

    bool ok_inv = true;
    for (int a = 1; a < 256; ++a)
    {
        uint8_t inv = gf.inverse(static_cast<uint8_t>(a));
        if (gf.multiply(static_cast<uint8_t>(a), inv) != 0x01)
        {
            ok_inv = false;
            cout << "Ошибка inverse при a=0x" << hex << a << endl;
            break;
        }
    }
    cout << "Проверка a * a^-1 = 1 для всех a: "
         << (ok_inv ? "OK" : "FAIL") << dec << endl;
    cout << endl;
    ...
    bool ok1 = GCM_AES128::verify_tag_constant_time(tag, tag);
    bool ok2 = GCM_AES128::verify_tag_constant_time(tag, invalid_tag);

    cout << "Проверка тега (совпадающий):  "
         << (ok1 ? "SUCCESS" : "FAILED") << endl;
    cout << "Проверка тега (повреждённый): "
         << (ok2 ? "SUCCESS (плохо!)" : "FAILED (отклонён, как надо)") << endl;
