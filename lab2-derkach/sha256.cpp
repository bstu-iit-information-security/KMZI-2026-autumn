...
void sha256_transform(SHA256_CTX* ctx, const uint8_t data[]) {
    ...
    for (i = 0; i < 16; ++i) {
        m[i] = (static_cast<WORD>(data[i * 4]) << 24) | (static_cast<WORD>(data[i * 4 + 1]) << 16) |
               (static_cast<WORD>(data[i * 4 + 2]) << 8) | (static_cast<WORD>(data[i * 4 + 3]));
    }
    for (; i < 64; ++i) {
...
void sha256_update(SHA256_CTX* ctx, const uint8_t data[], std::size_t len) {
...
bool sha256_selftest() {
    ...
    const uint8_t empty[kSha256Len] = {
        0xe3, 0xb0, 0xc4, 0x42, 0x98, 0xfc, 0x1c, 0x14, 0x9a, 0xfb, 0xf4, 0xc8, 0x99, 0x6f, 0xb9, 0x24,
        0x27, 0xae, 0x41, 0xe4, 0x64, 0x9b, 0x93, 0x4c, 0xa4, 0x95, 0x99, 0x1b, 0x78, 0x52, 0xb8, 0x55};
    if (std::memcmp(h, empty, kSha256Len) != 0) {
        return false;
    }
    const char* abc = "abc";
    sha256(reinterpret_cast<const uint8_t*>(abc), 3, h);
    const uint8_t abc_ref[kSha256Len] = {
        0xba, 0x78, 0x16, 0xbf, 0x8f, 0x01, 0xcf, 0xea, 0x41, 0x41, 0x40, 0xde, 0x5d, 0xae, 0x22, 0x23,
        0xb0, 0x03, 0x61, 0xa3, 0x96, 0x17, 0x7a, 0x9c, 0xb4, 0x10, 0xff, 0x61, 0xf2, 0x00, 0x15, 0xad};
    return std::memcmp(h, abc_ref, kSha256Len) == 0;
}
