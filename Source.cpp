...
public:
    ...
    BigInt(const string& hexStr)
    {
        digits.clear();
        string s = hexStr;
        if (s.length() % 8 != 0) s = string(8 - s.length() % 8, '0') + s;
    ...
    BigInt operator*(const BigInt& other) const
    {
        BigInt res; res.digits.assign(digits.size() + other.digits.size(), 0);
        for (size_t i = 0; i < digits.size(); ++i)
        {
            uint64_t carry = 0;
    ...
    {
        BigInt res; res.digits.resize(digits.size(), 0);
        uint32_t carry = 0;
        for (int i = (int)digits.size() - 1; i >= 0; --i)
        {
    ...
    pair<BigInt, BigInt> divmod(const BigInt& divisor) const
    {
        ...
        BigInt quotient = 0; BigInt remainder = 0;
        for (int i = (int)digits.size() - 1; i >= 0; --i)
        {
            for (int bit = 31; bit >= 0; --bit)
            {
                ...
                else { quotient = quotient.shiftLeft1(); }
            }
        }
    ...
    {
        stringstream ss; ss << hex << uppercase;
        ss << digits.back();
...
{
    BigInt m0 = m, t, q;
    BigInt x0 = 0, x1 = 1; BigInt zero = 0;
    if (m == BigInt(1)) return zero;
    while (a > BigInt(1))
    {
        auto dm = a.divmod(m0);
        q = dm.first; t = m0; m0 = dm.second; a = t;
        t = x0;
        BigInt qx0 = q * x0;
        if (x1 < qx0)
        {
...
vector<uint8_t> streebog256(const vector<uint8_t>& data)
{
    GOST34112012Context ctx;
    vector<uint8_t> hashResult(HASH_LEN);

    GOST34112012Init(&ctx, 256);
    if (!data.empty()) {
        GOST34112012Update(&ctx, data.data(), data.size());
    }
    GOST34112012Final(&ctx, hashResult.data());
...
vector<uint8_t> generateRandomBytes(size_t numBytes)
{
    ...
    vector<uint8_t> result; result.reserve(numBytes);
    while (result.size() < numBytes)
    {
        ...
        vector<uint8_t> hash = streebog256(seedData);
...
bool isPrimeMillerRabin(const BigInt& n, int k = 10)
{
    ...
    BigInt d = n - BigInt(1); size_t s = 0;
...
{
    vector<uint8_t> mask; uint32_t counter = 0;
    while (mask.size() < length)
    {
        vector<uint8_t> C(4);
        C[0] = (counter >> 24) & 0xFF; C[1] = (counter >> 16) & 0xFF;
        C[2] = (counter >> 8) & 0xFF;  C[3] = counter & 0xFF;
        vector<uint8_t> temp = seed;
        temp.insert(temp.end(), C.begin(), C.end());

        vector<uint8_t> hash = streebog256(temp);
        mask.insert(mask.end(), hash.begin(), hash.end());
        counter++;
    }
    mask.resize(length); return mask;
}
...
vector<uint8_t> oaepPad(const vector<uint8_t>& message, size_t k)
{
    ...
    vector<uint8_t> lHash = streebog256({});
    ...
    vector<uint8_t> em; em.reserve(k);
    em.push_back(0x00);
...
vector<uint8_t> oaepUnpad(const vector<uint8_t>& em, size_t k)
{
    ...
    while (msgStart < db.size() && db[msgStart] != 0x01) msgStart++;
    msgStart++;
...
BigInt rsaDecryptCRT(BigInt c, const RSAKey& key)
{
    ...
    BigInt validation = modPow(m, key.e, key.N);
    if (validation != c)
    {
        throw runtime_error("[!] ОШИБКА: Обнаружена Fault Injection атака! Валидация не пройдена.");
    }
    return m;
}
...
int main()
{
    size_t bitLength = 576;

    cout << "[+] Запуск генерации ключей (" << bitLength << " бит)..." << endl;
    ...
    size_t k = bitLength / 8;

    cout << "[1] Исходный текст: \"" << inputStr << "\"" << endl;
    vector<uint8_t> padded = oaepPad(message, k);

    BigInt msgNum = 0;
    for (size_t i = 0; i < min(padded.size(), static_cast<size_t>(64)); ++i)
    {
        msgNum = msgNum * BigInt(256) + BigInt(padded[i]);
    }

    BigInt cipherNum = rsaEncrypt(msgNum, key.e, key.N);
    cout << "[2] Зашифрованное значение (HEX): 0x" << cipherNum.toHexString() << endl;

    try {
        BigInt decryptedNum = rsaDecryptCRT(cipherNum, key);
        cout << "[3] Расшифрованное значение с проверкой FI (HEX): 0x" << decryptedNum.toHexString() << endl;

        vector<uint8_t> decryptedBytes(k, 0);
        BigInt temp = decryptedNum;
        for (int i = k - 1; i >= 0; --i) {
            decryptedBytes[i] = (temp % BigInt(256)).digits[0];
            temp = temp / BigInt(256);
        }

        vector<uint8_t> unpaddedMessage = oaepUnpad(decryptedBytes, k);
        string restoredStr(unpaddedMessage.begin(), unpaddedMessage.end());
        cout << "[4] Восстановленный текст из OAEP: \"" << restoredStr << "\"" << endl << endl;

        if (restoredStr == inputStr) {
            cout << "[УСПЕХ] Математика и защиты работают корректно!" << endl;
        }
        else {
